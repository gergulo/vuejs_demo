...
export default {
  ...
  methods: {
    getPermission: function(userInfo) {
      if(!userInfo || !userInfo.access || !userInfo.access.resources){
        return console.warn(userInfo);
      }
      let resourcePermission = {};
      let resources = userInfo.access.resources;
    ...
    getRoutes: function(userInfo) {
      if(!userInfo || !userInfo.access || !userInfo.access.menus){
        return console.warn(userInfo);
      }
      let vm = this;
      let allowedRouter = [];
      let menus = userInfo.access.menus;
      //将菜单数据转成多维数组格式
